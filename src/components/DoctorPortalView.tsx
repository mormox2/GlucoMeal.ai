        ...
        <div className="p-8 sm:p-12 rounded-3xl bg-white border border-slate-200/80 shadow-md text-center max-w-md mx-auto space-y-4">
          ...
          <div>
            <h2 className="text-lg font-black text-slate-900">
              {isAr ? 'دخول مخصص للطبيب المعالج' : 'Accès Réservé au Praticien'}
            </h2>
            <p className="text-xs text-slate-500 mt-1">
              {isAr
                ? 'أدخل رمز الدخول الطبي أو انقر مباشرة على "دخول تجريبي فوري".'
          ...
          <form onSubmit={handleUnlock} className="space-y-3 pt-2">
            <input
              type="text"
              placeholder={isAr ? 'رمز الطبيب (مثال: DR-GLUCO-2026)' : 'Code consultation (ex: DR-GLUCO-2026)'}
              value={doctorCode}
            ...
            {codeError && (
              <p className="text-xs text-rose-600 font-medium">
                {isAr ? 'رمز غير صالح. استخدم DR-GLUCO-2026.' : 'Code invalide. Utilisez DR-GLUCO-2026.'}
              </p>
            )}
            ...
            >
              ...
              <span>{isAr ? 'فتح البوابة للاستشارة' : 'Déverrouiller pour la consultation'}</span>
            </button>
            ...
            >
              {isAr ? 'دخول تجريبي فوري (DR-GLUCO-2026)' : 'Accès démo immédiat (DR-GLUCO-2026)'}
            </button>
          </form>
        </div>
      ) : (
        ...
        <div className="space-y-6">
          ...
          {report?.honeymoonInsight && (
            <div className={`p-5 rounded-3xl border flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-xs ${
              ...
              >
                {userProfile.isHoneymoonPhase
                  ? (isAr ? 'تعطيل وضع شهر العسل' : 'Désactiver Mode Lune de Miel')
                  : (isAr ? 'تفعيل وضع شهر العسل' : 'Activer Mode Lune de Miel')}
              </button>
            </div>
          )}
                ...
                return (
                  ...
                  >
                    ...
                    {hasRecommendation && (
                      ...
                      >
                        {isAr
                          ? `اعتماد المعامل الجديد : 1 وحدة / ${info.suggestedRatio} غ`
