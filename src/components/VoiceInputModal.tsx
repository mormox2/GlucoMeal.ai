...
import { Mic, MicOff, X, Sparkles, RefreshCw, Volume2, Globe } from 'lucide-react';
...
export const VoiceInputModal: React.FC<VoiceInputModalProps> = ({
  ...
  const [language, setLanguage] = useState<'fr-FR' | 'ar-TN'>('fr-FR');
  ...
  const spokenTunisianExamples = [
    { text: 'كلّيت صحن مقرونة و زوز خبزات', lang: 'ar-TN', label: 'Derja : Pâtes + 2 morceaux de pain' },
    { text: 'صحن لبلابي مع عظمة و شوية تن', lang: 'ar-TN', label: 'Derja : Lablabi complet + œuf + thon' },
    { text: 'J’ai mangé un plat de couscous, deux morceaux de pain et une orange', lang: 'fr-FR', label: 'FR : Couscous + 2 pains + orange' },
    { text: '2 tranches de pain tabouna avec une ojja merguez et un café', lang: 'fr-FR', label: 'FR : Tabouna + ojja + café' },
  ];
        ...
        <div className="p-6 flex flex-col items-center justify-center text-center">
          ...
          <p className="text-xs font-medium text-slate-600">
            {isRecording
              ...
              : 'Touchez le micro pour commencer à parler'}
          </p>

          {/* Transcript Box */}
          <div className="w-full mt-5 p-4 rounded-2xl bg-slate-50 border border-slate-200/80 text-left min-h-20 flex flex-col justify-center">
            {transcript ? (
              ...
              <p className="text-xs text-slate-400 italic text-center">
                Votre description vocale apparaîtra ici…
              </p>
            )}
          </div>
          ...
          <div className="w-full mt-5 text-left">
            <span className="text-[11px] font-semibold text-slate-500 block mb-2">
              Ou simuler un message vocal tunisien typique :
            </span>
            <div className="space-y-1.5">
              {spokenTunisianExamples.map((ex, i) => (
                <button
                  ...
                  className="w-full p-2 rounded-xl bg-teal-50/50 hover:bg-teal-50 text-slate-800 text-xs flex items-center justify-between border border-teal-100/70 transition-colors text-left cursor-pointer"
                >
                  <span className="font-medium">« {ex.text} »</span>
                  <span className="text-[10px] text-teal-700 font-semibold shrink-0 ml-2">
                    {ex.label}
                  </span>
                </button>
              ))}
          ...
          >
            {isAnalyzing ? (
              ...
              <>
                ...
                <span>Estimer les glucides</span>
              </>
            )}
          </button>
        </div>
