...
import { Database, Search, Sparkles, Activity, X, ChevronRight } from 'lucide-react';
...
export const FoodDatabaseView: React.FC = () => {
  ...
  const categories: { id: string; label: string; count: number }[] = [
    ...
    { id: 'patisseries', label: isAr ? '4. حلويات ومخبوزات' : '4. Pâtisseries & Douceurs', count: TUNISIAN_FOOD_DATABASE.filter((f) => f.category === 'patisseries').length },
    { id: 'fruits_legumes', label: isAr ? '5. غلال وخضار' : '5. Fruits & Légumes', count: TUNISIAN_FOOD_DATABASE.filter((f) => f.category === 'fruits_legumes').length },
    { id: 'produits_industriels', label: isAr ? '6. منتجات مصنعة' : '6. Produits industriels', count: TUNISIAN_FOOD_DATABASE.filter((f) => f.category === 'produits_industriels').length },
    { id: 'boissons', label: isAr ? '7. مشروبات' : '7. Boissons', count: TUNISIAN_FOOD_DATABASE.filter((f) => f.category === 'boissons').length },
  ];

  const quickSearches = isAr
    ? [
        { label: 'كسكسي', query: 'couscous' },
        { label: 'كفتاجي', query: 'kafteji' },
        { label: 'مقرونة', query: 'makrouna' },
        { label: 'طابونة', query: 'tabouna' },
        { label: 'بريك', query: 'brik' },
        { label: 'عجة', query: 'ojja' },
        { label: 'شربة', query: 'chorba' },
        { label: 'لبلابي', query: 'lablabi' },
        { label: 'مقروض', query: 'makroudh' },
      ]
    : [
        { label: 'Couscous', query: 'Couscous' },
        { label: 'Kafteji', query: 'Kafteji' },
        { label: 'Makrouna', query: 'Makrouna' },
        { label: 'Tabouna', query: 'Tabouna' },
        { label: 'Brik', query: 'Brik' },
        { label: 'Ojja', query: 'Ojja' },
        { label: 'Chorba', query: 'Chorba' },
        { label: 'Lablabi', query: 'Lablabi' },
        { label: 'Makroudh', query: 'Makroudh' },
      ];

  const getCategoryLabel = (cat: string) => {
    if (!isAr) return cat;
    switch (cat) {
      case 'plats': return 'أطباق';
      case 'feculents': return 'نشويات';
      case 'legumineuses': return 'بقوليات';
      case 'patisseries': return 'حلويات';
      case 'fruits_legumes': return 'خضار وغلال';
      case 'produits_industriels': return 'مصنع';
      case 'boissons': return 'مشروبات';
      case 'viandes_proteines': return 'بروتينات';
      default: return cat;
    }
  };
  ...
  return (
    <div className={`max-w-6xl 2xl:max-w-7xl mx-auto py-8 sm:py-10 px-4 sm:px-6 lg:px-8 ${isAr ? 'font-arabic' : ''}`}>
      {/* Header */}
      ...
      <div className="bg-white p-4 rounded-3xl border border-slate-200/80 shadow-xs mb-6 space-y-3">
        <div className="relative">
          ...
          <input
            ...
            placeholder={
              isAr
                ? 'ابحث عن أكلة: كفتاجي، مقرونة، ملوخية، بريك، فريكاسي، شربة، طابونة، كسكسي...'
                : 'Rechercher : kafteji, makrouna, mloukhiya, brik, fricassé, chorba, tabouna, couscous...'
            }
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            className={`w-full ${isAr ? 'pr-10 pl-10 text-right' : 'pl-10 pr-10 text-left'} py-2.5 rounded-2xl border border-slate-200 text-xs sm:text-sm text-slate-800 outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/10`}
          />
          {searchQuery && (
            <button
              onClick={() => setSearchQuery('')}
              className={`absolute ${isAr ? 'left-3' : 'right-3'} top-1/2 -translate-y-1/2 p-1 text-slate-400 hover:text-slate-600 rounded-full hover:bg-slate-100 cursor-pointer`}
              title={isAr ? 'مسح البحث' : 'Effacer la recherche'}
            >
        ...
        <div className="flex items-center gap-1.5 overflow-x-auto pb-1 scrollbar-none text-[11px]">
          <span className="text-slate-400 font-bold shrink-0 flex items-center gap-1">
            ...
            {isAr ? 'اقتراحات شائعة :' : 'Suggestions rapides :'}
          </span>
        ...
        <div className="flex items-center gap-2 pt-1 text-xs flex-wrap border-t border-slate-100">
          <span className="font-semibold text-slate-500 flex items-center gap-1">
            ...
            {isAr ? 'مؤشر السكر (IG):' : 'Filtre Index Glycémique :'}
          </span>
          <div className="flex items-center gap-1.5">
            ...
            >
              ...
              <span>{isAr ? 'مؤشر منخفض (< 55)' : 'IG Bas (< 55)'}</span>
            </button>
            <button
            ...
            >
              ...
              <span>{isAr ? 'مؤشر متوسط (55-69)' : 'IG Moyen (55-69)'}</span>
            </button>
            <button
            ...
            >
              ...
              <span>{isAr ? 'مؤشر مرتفع (≥ 70)' : 'IG Élevé (≥ 70)'}</span>
            </button>
          </div>
        </div>
      </div>
        ...
        <div className="divide-y divide-slate-100 max-h-[600px] overflow-y-auto">
          {filteredFoods.length > 0 ? (
            filteredFoods.map((food) => {
              ...
              const igLabel = isAr
                ? (igCat.color === 'emerald' ? 'منخفض' : igCat.color === 'amber' ? 'متوسط' : 'مرتفع')
                : igCat.label;
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      <span className="text-sm font-bold text-slate-900 group-hover:text-emerald-700 transition-colors">
                        {isAr ? (food.name_ar || food.name_fr) : food.name_fr}
                      </span>
                      {isAr ? (
                        food.name_fr && (
                    ...
                    <p className="text-xs text-slate-500 mt-1">
                      {isAr ? (
                        <>
                          التسمية بالدارجة: <span className="font-semibold text-slate-700">{food.name_tn}</span> • الحصة: {food.serving_unit_description}
                        </>
                      ) : (
                        <>
                  ...
                  <div className="flex items-center gap-3 shrink-0">
                    <div className={isAr ? 'text-left' : 'text-right'}>
                      <span className="text-xs font-bold text-slate-800 block">
                        {isAr ? `${food.carbs_per_100g} غ / 100 غ` : `${food.carbs_per_100g} g / 100 g`}
                      </span>
                      <span className="text-[11px] text-slate-500">
                        {isAr ? `الحصة: ${food.default_portion_g} غ` : `Portion : ${food.default_portion_g} g`}
                      </span>
                    </div>

                    <div className={`px-3 py-1.5 rounded-xl bg-emerald-50 border border-emerald-200 ${isAr ? 'text-left' : 'text-right'} min-w-22`}>
                      <span className="text-xs font-extrabold text-emerald-800 block">
                        ≈ {carbsPerPortion} {isAr ? 'غ' : 'g'}
                      </span>
                      <span className="text-[10px] text-emerald-600 font-medium">
                        {isAr ? 'في الحصة' : 'par portion'}
                      </span>
                    </div>

                    <ChevronRight className={`w-4 h-4 text-slate-300 group-hover:text-emerald-600 transition-colors ${isAr ? 'rotate-180' : ''}`} />
                  </div>
                </div>
              );
            })
            ...
            <div className="p-8 sm:p-10 text-center space-y-4">
              ...
              <div className="space-y-1">
                <h4 className="text-sm font-black text-slate-800">
                  {isAr
                    ? `لم يتم العثور على أطعمة ${searchQuery ? `لـ « ${searchQuery} »` : 'بهذه المعايير'}`
            ...
            <div className="p-5 border-b border-slate-100 flex items-start justify-between">
              <div>
                <span className="text-[11px] font-bold text-emerald-700 uppercase tracking-wider block mb-1">
                  {isAr ? 'البطاقة الغذائية للأطباق التونسية' : 'Fiche Nutritionnelle Spécialité Tunisienne'}
                </span>
                <h3 className="text-lg font-black text-slate-900">
                  {isAr ? (inspectedFood.name_ar || inspectedFood.name_fr) : inspectedFood.name_fr}
                </h3>
                <p className="text-xs text-slate-500">
                  {isAr
                    ? `${inspectedFood.name_fr} • ${inspectedFood.name_tn}`
            ...
            <div className="p-5 space-y-4 text-xs">
              ...
              <div className="p-3.5 rounded-2xl bg-slate-50 border border-slate-200/80">
                <span className="text-slate-500 block text-[11px]">
                  {isAr ? 'الحصة النمطية الموصى بها:' : 'Portion type recommandée :'}
                </span>
              ...
              <div className="grid grid-cols-4 gap-2 text-center">
                <div className="p-2.5 rounded-2xl bg-emerald-50 border border-emerald-200">
                  <span className="text-[10px] font-bold text-emerald-700 block uppercase">
                    {isAr ? 'كربوهيدرات' : 'Glucides'}
                  </span>
                  <span className="text-base font-black text-emerald-900">
                    {inspectedFood.carbs_per_100g} {isAr ? 'غ' : 'g'}
                  </span>
                  <span className="text-[10px] text-emerald-600">{isAr ? 'لكل 100 غ' : '/ 100 g'}</span>
                </div>
                <div className="p-2.5 rounded-2xl bg-blue-50 border border-blue-200">
                  <span className="text-[10px] font-bold text-blue-700 block uppercase">
                    {isAr ? 'بروتينات' : 'Protéines'}
                  </span>
                  <span className="text-base font-black text-blue-900">
                    {inspectedFood.protein_per_100g} {isAr ? 'غ' : 'g'}
                  </span>
                  <span className="text-[10px] text-blue-600">{isAr ? 'لكل 100 غ' : '/ 100 g'}</span>
                </div>
                <div className="p-2.5 rounded-2xl bg-amber-50 border border-amber-200">
                  ...
                  <span className="text-base font-black text-amber-900">
                    {inspectedFood.fat_per_100g} {isAr ? 'غ' : 'g'}
                  </span>
                  <span className="text-[10px] text-amber-600">{isAr ? 'لكل 100 غ' : '/ 100 g'}</span>
                </div>
                <div className="p-2.5 rounded-2xl bg-purple-50 border border-purple-200">
                  ...
                  <span className="text-base font-black text-purple-900">
                    {inspectedFood.fiber_per_100g} {isAr ? 'غ' : 'g'}
                  </span>
                  <span className="text-[10px] text-purple-600">{isAr ? 'لكل 100 غ' : '/ 100 g'}</span>
                </div>
              </div>
              ...
              <div className="p-4 rounded-2xl bg-slate-900 text-white space-y-2">
                <div className="flex items-center justify-between">
                  <span className="text-[11px] font-bold text-emerald-400 flex items-center gap-1.5">
                    ...
                    {isAr ? 'التأثير السكري للسكري نوع 1' : 'Impact Glycémique DT1'}
                  </span>
                  <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-white/20 text-white">
                    {isAr ? `المؤشر السكري: ${inspectedFood.glycemic_index || 50}` : `Index Glycémique : ${inspectedFood.glycemic_index || 50}`}
                  </span>
                </div>

                <div className="flex items-center justify-between text-xs pt-1 border-t border-white/10">
                  <span className="text-slate-300">
                    {isAr ? 'الحمل السكري للحصة:' : 'Charge glycémique (CG) par portion :'}
                  </span>
                ...
                <p className="text-[11px] text-slate-300 leading-relaxed pt-1">
                  {isAr
                    ? (inspectedFood.glycemic_index && inspectedFood.glycemic_index >= 70
                        ? 'مؤشر سكري مرتفع: يسبب ارتفاعاً سريعاً في السكر، يفضل حقن الإنسولين قبل الأكل بـ 15 دقيقة.'
                        : inspectedFood.glycemic_index && inspectedFood.glycemic_index >= 55
                        ? 'مؤشر سكري متوسط: امتصاص متوازن للكربوهيدرات.'
                        : 'مؤشر سكري منخفض: امتصاص بطيء يحافظ على استقرار السكر.')
                    : getGlycemicCategory(inspectedFood.glycemic_index).description}
                  {inspectedFood.id === 'plat-mloukhiya-01' && (isAr ? ' • تنبيه DT1: الدهون الكثيفة في الملوخية تؤخر امتصاص الخبز، يُنصح بتمديد جزء من الجرعة عبر المضخة.' : " Note DT1 : Les lipides abondants de la mloukhiya ralentissent l'absorption du pain d'accompagnement (bolus carré/mixte recommandé sous pompe).")}
                  {inspectedFood.id === 'plat-makrouna-salsa-01' && (isAr ? ' • تنبيه DT1: نسبة كربوهيدرات مرتفعة، راقب السكر بعد ساعتين إلى 3 ساعات.' : " Note DT1 : Teneur glucidique élevée, surveiller la glycémie 2h à 3h après le repas.")}
                  {(inspectedFood.category === 'viandes_proteines' || inspectedFood.carbs_per_100g === 0) && (isAr ? ' • تنبيه DT1: اللحوم الصافية لا تحتوي على كربوهيدرات مباشرة، ولكن البروتينات والدهون تؤخر الهضم وتتطلب مراقبة السكر المتأخر.' : " Note DT1 : Les protéines pures ne contiennent pas de glucides directs. Cependant, un apport protéique et lipidique important retarde la vidange gastrique et peut nécessiter un bolus prolongé (dual-wave) 2h à 4h après le repas.")}
                </p>
              </div>
